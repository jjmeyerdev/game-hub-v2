  ...
  return (
    <div className="flex-1 space-y-4">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h3 className="font-semibold text-[var(--theme-text-primary)] font-[family-name:var(--font-family-display)]">
            {profile.username}
          </h3>
      ...
      {platform === 'psn' && profile.stats.platformSpecific.trophyLevel !== undefined && (
        <div className={`flex items-center gap-3 p-3 rounded-lg ${colors.bg} ${colors.border} border`}>
          ...
          <div>
            <div className="text-xs text-[var(--theme-text-subtle)]">Trophy Level</div>
      ...
      {platform === 'xbox' && profile.stats.platformSpecific.gamerscore !== undefined && (
        <div className={`flex items-center gap-3 p-3 rounded-lg ${colors.bg} ${colors.border} border`}>
          <Star className={`w-5 h-5 ${colors.accent}`} />
          <div>
            <div className="text-xs text-[var(--theme-text-subtle)]">Gamerscore</div>
            <div className={`text-lg font-bold font-mono ${colors.accent}`}>
              {profile.stats.platformSpecific.gamerscore?.toLocaleString() || 0}
            </div>
          </div>
          {profile.stats.platformSpecific.tier && (
            <div className="ml-auto px-2 py-1 bg-[var(--theme-hover-bg)] border border-[var(--theme-border)] rounded-lg">
...
function StatBox({ icon: Icon, label, value, platform }: StatBoxProps) {
  ...
  return (
    <div className="p-3 bg-[var(--theme-bg-tertiary)] border border-[var(--theme-border)] rounded-lg">
      <div className="flex items-center gap-2 mb-1">
        <Icon className={`w-4 h-4 ${colors.accent}`} />
        <span className="text-[10px] font-mono text-[var(--theme-text-subtle)] uppercase tracking-wider">{label}</span>
      </div>
      <div className="text-xl font-bold font-mono text-[var(--theme-text-primary)]">{value}</div>
    </div>
  );
}
...
function LoadingColumn() {
  return (
    <div className="flex-1 space-y-4 animate-pulse">
      <div className="flex items-center gap-3">
        <div className="w-14 h-14 rounded-xl bg-[var(--theme-hover-bg)]" />
        <div className="space-y-2">
          <div className="h-4 w-24 rounded bg-[var(--theme-hover-bg)]" />
          <div className="h-3 w-16 rounded bg-[var(--theme-hover-bg)]" />
        </div>
      </div>
      <div className="grid grid-cols-2 gap-3">
        {[1, 2, 3, 4].map((i) => (
          <div key={i} className="h-20 rounded-lg bg-[var(--theme-hover-bg)]" />
        ))}
      </div>
    </div>
  );
...
function EmptyColumn({ platform }: { platform: ComparePlatform }) {
  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-center py-8 text-center">
      ...
      <p className="text-sm text-[var(--theme-text-muted)] mb-1">Search for a friend</p>
...
export function ComparisonCard({ user, friend, platform, isLoading }: ComparisonCardProps) {
  ...
  return (
    <div className="relative bg-[var(--theme-bg-secondary)] border border-[var(--theme-border)] rounded-2xl p-6 overflow-hidden">
      {/* HUD corners */}
      ...
      <div className="flex gap-8">
        ...
        <div className="flex flex-col items-center justify-center">
          <div className="w-px h-16 bg-gradient-to-b from-transparent via-[var(--theme-border)] to-transparent" />
          ...
          <div className="w-px h-16 bg-gradient-to-b from-[var(--theme-border)] via-[var(--theme-border)] to-transparent" />
        </div>
