    ...
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="relative">
        <div className="flex items-center gap-4 mb-2">
          ...
          <div>
            <h2 className="text-2xl font-bold text-white tracking-wide">Notification Settings</h2>
            <p className="text-gray-400 text-sm">Control how and when you receive alerts</p>
          </div>
        </div>
      ...
      <div className={`relative group transition-opacity duration-300 ${!masterEnabled ? 'opacity-50 pointer-events-none' : ''}`}>
        ...
        <div className="relative bg-abyss/60 backdrop-blur-sm border border-steel/30 rounded-xl p-5">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h4 className="font-semibold text-white">Notification Sounds</h4>
                <p className="text-xs text-gray-500">Play sounds for in-app notifications</p>
              </div>
            </div>
            <button
        ...
        <div className="grid gap-4">
          {categories.map((category, index) => (
            ...
            >
              ...
              <div className={`relative bg-abyss/90 backdrop-blur-sm border rounded-xl overflow-hidden transition-all duration-300 ${
                category.enabled ? 'border-steel/50' : 'border-steel/20'
              }`}>
                {/* Top accent line */}
                ...
                <div className="p-5">
                  <div className="flex items-start gap-4">
                    ...
                    <div className={`w-10 h-10 rounded-lg border flex items-center justify-center flex-shrink-0 transition-all duration-300 ${
                      category.enabled
                        ? 'bg-deep/50 border-steel/50'
                        : 'bg-deep/30 border-steel/20'
                    }`}>
                      {category.icon}
                    </div>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between mb-1">
                        <h4 className={`font-semibold transition-colors ${
                          category.enabled ? 'text-white' : 'text-gray-500'
                        }`}>
                          {category.title}
                        </h4>
                      ...
                      <p className={`text-sm mb-4 transition-colors ${
                        category.enabled ? 'text-gray-400' : 'text-gray-600'
                      }`}>
                        ...
                        <button
                          ...
                          className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${
                            category.channels.email
                              ...
                              : 'bg-deep/50 text-gray-500 border border-steel/30 hover:border-steel/50'
                          }`}
                        >
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <input
                    type="time"
                    defaultValue="22:00"
                    className="w-full px-4 py-3 bg-deep/50 border border-steel/30 rounded-xl text-white focus:outline-none focus:border-cyan-500/50 focus:ring-2 focus:ring-cyan-500/20 transition-all"
                  />
                </div>
              </div>
              <div className="space-y-2">
      ...
      <div className="flex justify-end pt-2">
        <button
          onClick={handleSave}
          disabled={saving}
          className="relative px-6 py-2.5 bg-gradient-to-r from-purple-500 to-purple-600 hover:from-purple-400 hover:to-purple-500 rounded-xl font-semibold text-white transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2 overflow-hidden group/btn"
        >
