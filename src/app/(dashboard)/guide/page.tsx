  ...
  return (
    <div className="relative min-h-screen bg-void">
      ...
      <header className="relative border-b border-white/[0.06]">
        ...
        <div className="px-6 lg:px-8 py-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <div className="flex items-center gap-2 mb-1">
                  <span className="text-[10px] font-mono text-white/30 uppercase tracking-wider">// SETUP_MANUAL</span>
                </div>
            ...
            <Link
              href="/settings"
              className="group flex items-center gap-2 px-4 py-2 bg-white/[0.03] hover:bg-amber-500/10 border border-white/[0.08] hover:border-amber-500/30 rounded-xl text-sm text-white/50 hover:text-amber-400 transition-all"
            >
      ...
      <div className="max-w-4xl mx-auto px-6 lg:px-8 py-10 space-y-10">
        ...
        {/* Introduction */}
        <section className="relative p-6 bg-abyss border border-cyan-500/20 rounded-xl overflow-hidden">
          {/* HUD corners */}
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              <div className="flex items-center gap-2 mb-2">
                <h2 className="text-lg font-bold text-white uppercase tracking-wide font-[family-name:var(--font-family-display)]">
                  Getting Started
                </h2>
                <span className="text-[10px] font-mono text-white/30">// INIT_SEQUENCE</span>
              </div>
        ...
        <section>
          <div className="flex items-center gap-3 mb-4">
            ...
            <span className="text-[10px] font-mono text-white/30">// AUTH_METHODS</span>
        ...
        {/* Footer */}
        <footer className="relative pt-6 border-t border-white/[0.06]">
          {/* HUD corners */}
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <p className="text-[11px] font-mono text-white/30 uppercase tracking-wider">
                  Guide v1.0 <span className="text-white/20">•</span> Documentation
                </p>
              </div>
            </div>
            <Link
              href="/settings"
              className="group flex items-center gap-1.5 text-[11px] font-mono text-cyan-400 hover:text-cyan-300 transition-colors uppercase tracking-wider"
            >
              Return to Settings
...
function PlatformSection({ id, name, tagline, color, difficulty, time, icon, requirements, steps, tips, warnings }: PlatformSectionProps) {
  ...
  return (
    <section id={id} className="scroll-mt-24">
      <div className={`relative bg-abyss border ${c.border} rounded-xl overflow-hidden`}>
        {/* HUD corners */}
        ...
        {/* Header */}
        <div className={`px-6 py-4 ${c.headerBg} border-b border-white/[0.06]`}>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="flex items-center gap-2">
                  <h3 className="text-lg font-bold text-white uppercase tracking-wide font-[family-name:var(--font-family-display)]">
                    {name}
                  </h3>
                ...
                <p className="text-[11px] font-mono text-white/40 uppercase tracking-wider">{tagline}</p>
              </div>
            </div>
            <div className="flex items-center gap-4">
              <div className="text-right">
                <p className="text-[9px] font-mono text-white/30 uppercase tracking-wider mb-1">Difficulty</p>
              ...
              <div className="text-right">
                <p className="text-[9px] font-mono text-white/30 uppercase tracking-wider mb-1">Time</p>
                <p className="text-xs font-mono font-medium text-white">{time}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="p-6 space-y-6">
          {/* Requirements */}
          <div className="relative p-4 bg-white/[0.02] rounded-lg border border-white/[0.06] overflow-hidden">
            <div className="absolute top-0 left-0 w-2 h-2 border-l border-t border-white/[0.1]" />
            <div className="absolute top-0 right-0 w-2 h-2 border-r border-t border-white/[0.1]" />
            <h4 className="text-[10px] font-mono font-medium text-white/40 uppercase tracking-wider mb-3 flex items-center gap-2">
          ...
          {/* Steps */}
          <div>
            <h4 className="text-[10px] font-mono font-medium text-white/40 uppercase tracking-wider mb-4 flex items-center gap-2">
              ...
              {steps.map((step, i) => (
                <div key={i} className="flex gap-3">
                  ...
                  <div className="flex-1 pt-0.5">
                    <h5 className="text-sm font-semibold text-white mb-0.5">{step.title}</h5>
...
function QuickRefCard({ platform, method, color, icon }: QuickRefCardProps) {
  const colorMap = {
    ...
    gray: { border: 'border-white/[0.08]', text: 'text-white/50', bg: 'bg-white/[0.02]', corner: 'border-white/[0.12]' },
  };

  const c = colorMap[color];

  return (
    <div className={`group relative flex items-center gap-3 p-3 bg-abyss ${c.bg} border ${c.border} rounded-lg hover:border-white/[0.15] transition-all`}>
      ...
      <div>
        ...
        <p className="text-[10px] font-mono text-white/30 uppercase tracking-wider">{method}</p>
      </div>
    </div>
  );
}
