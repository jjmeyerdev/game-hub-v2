  ...
  try {
    ...
    for (const epicGame of validGames) {
      try {
        ...
        // This prevents duplicates even if game was previously added with different platform string
        ...
        if (existingUserGame) {
          // For existing games, only update timestamp
          // Epic doesn't provide playtime or achievement data, so we just update the sync timestamp
          // Respects locked_fields for consistency
          const { error: updateError } = await supabase
            .from('user_games')
            .update({
              updated_at: new Date().toISOString(),
